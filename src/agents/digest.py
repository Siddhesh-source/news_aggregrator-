...
def send_digest(articles, save_preview=False):
    """
    Send a digest email with top 5 articles sorted by prelims_score.
    Includes detailed PDF analysis as attachment.
    ...
    # Build HTML email
    html_content = build_html_email(top_articles, category_counts)
    ...
    msg['Subject'] = f'UPSC Daily Intelligence - {len(top_articles)} Articles'
...
def build_html_email(articles, category_counts):
    """
    ...
    for idx, article in enumerate(articles):
        ...
        # Check if title is in Hindi
        title_html = ""
        if is_hindi(title):
            # For Hindi titles, we need English translation from Gemini
            title_english = article.get('title_english', title)
            title_html = f"""
                <h2 style="color: #1a1a2e; font-size: 22px; font-family: Georgia, serif; line-height: 1.35; font-weight: bold; margin: 10px 0 4px 0;">
                    {title_english}
                </h2>
                <div style="color: #999999; font-size: 14px; font-family: Georgia, serif; font-style: italic; margin-top: 4px;">
                    {title}
                </div>
            """
        else:
